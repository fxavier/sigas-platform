import { NextResponse } from 'next/server';
import { createContextualPrismaClient } from '@/lib/db-context';

export async function GET(req: Request) {
  try {
    const { searchParams } = new URL(req.url);
    const tenantId = searchParams.get('tenantId');

    if (!tenantId) {
      return NextResponse.json(
        { error: 'tenantId é obrigatório' },
        { status: 400 }
      );
    }

    const contextualPrisma = createContextualPrismaClient({
      tenantId: tenantId || undefined,
    });

    const areas = await contextualPrisma.areaTreinamento.findMany({
      where: {
        tenantId,
      },
      orderBy: {
        name: 'asc',
      },
    });

    return NextResponse.json(areas);
  } catch (error) {
    console.error('Error fetching areas treinamento:', error);
    return NextResponse.json(
      { error: 'Erro ao buscar áreas de treinamento' },
      { status: 500 }
    );
  }
}

export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { name, tenantId } = body;

    if (!name || !tenantId) {
      return NextResponse.json(
        { error: 'Nome e tenantId são obrigatórios' },
        { status: 400 }
      );
    }

    const contextualPrisma = createContextualPrismaClient({
      tenantId: tenantId || undefined,
    });

    const area = await contextualPrisma.areaTreinamento.create({
      data: {
        name,
        tenantId,
      },
    });

    return NextResponse.json(area, { status: 201 });
  } catch (error) {
    console.error('Error creating area treinamento:', error);
    return NextResponse.json(
      { error: 'Erro ao criar área de treinamento' },
      { status: 500 }
    );
  }
}
